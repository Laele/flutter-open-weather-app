import 'package:flutter/material.dart';
import 'package:flutter_animate/flutter_animate.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_weather_app/features/home/presentation/current_weather_bloc/current_weather_bloc.dart';
import 'package:flutter_weather_app/core/common/utils/get_weather_animation.dart';
import 'package:flutter_weather_app/features/home/presentation/widgets/app_gradient_container.dart';

import 'package:lottie/lottie.dart';

class WeatherAnimationSection extends StatelessWidget {
  const WeatherAnimationSection({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<CurrentWeatherBloc, CurrentWeatherState>(
      builder: (context, state) {
        if (state is CurrentWeatherSuccess) {
          return WeatherAnimationContainer(
            currentWeatherWidget: ExpandedWeatherValueText(weatherValue: state.weather.mainTemp.temp.toInt()),
            iconCode: state.weather.mainWeather.icon,
            tempMin: state.weather.mainTemp.tempMin.toInt().toString(),
            tempMax: state.weather.mainTemp.tempMax.toInt().toString(),
          );
        } else if (state is CurrentWeatherInitial || state is CurrentWeatherLoading) {
          return WeatherAnimationContainer(
            currentWeatherWidget: Expanded(child: Container()),
            iconCode: '',
            tempMax: '',
            tempMin: '',
          );
        }
        // Errror State
        return WeatherAnimationContainer(
          currentWeatherWidget: Expanded(child: Container()),
          iconCode: '',
          tempMax: '-',
          tempMin: '-',
        );
      },
    );
  }
}

class ExpandedWeatherValueText extends StatelessWidget {
  final int weatherValue;
  const ExpandedWeatherValueText({super.key, required this.weatherValue});

  @override
  Widget build(BuildContext context) {
    return Expanded(
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.baseline,
        textBaseline: TextBaseline.alphabetic,
        children: [
          Expanded(
            child: FittedBox(
              fit: BoxFit.contain,
              child: TweenAnimationBuilder<int>(
                tween: IntTween(begin: 0, end: weatherValue),
                duration: Duration(seconds: 1),
                curve: Curves.easeOut,
                builder: (context, value, child) => Text(
                  '${value.toInt()}',
                  style: Theme.of(context).textTheme.displayLarge!.copyWith(fontWeight: FontWeight.bold, color: Colors.white),
                ),
              ).animate().fade(duration: Duration(seconds: 3)),
            ),
          ),
        ],
      ),
    );
  }
}

class WeatherAnimationContainer extends StatelessWidget {
  final Widget currentWeatherWidget;
  final String tempMin;
  final String tempMax;
  final String iconCode;
  const WeatherAnimationContainer({super.key, required this.currentWeatherWidget, required this.iconCode, required this.tempMin, required this.tempMax});

  @override
  Widget build(BuildContext context) {
    return Container(
      constraints: BoxConstraints(maxWidth: 600),
      child: Center(
        child: AspectRatio(
          aspectRatio: 1,
          child: Stack(
            children: [
              LayoutBuilder(
                builder: (context, constraints) => Stack(
                  children: [
                    Center(
                      child: FractionallySizedBox(
                        widthFactor: 0.8,
                        heightFactor: 0.8,
                        child: AppGradientContainer(
                          child: Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Center(
                              child: Column(
                                children: [
                                  // Current Weather
                                  currentWeatherWidget,
                                  // Extra Info
                                  Align(
                                    alignment: Alignment.bottomRight,
                                    child: FractionallySizedBox(
                                      widthFactor: 0.5,
                                      child: Row(
                                        children: [
                                          Expanded(
                                            child: FittedBox(
                                              child: Row(
                                                mainAxisAlignment: MainAxisAlignment.end,
                                                children: [
                                                  Text(
                                                    'L: $tempMin °C',
                                                    style: Theme.of(context).textTheme.bodyMedium!.copyWith(color: Colors.white, fontWeight: FontWeight.bold),
                                                  ).animate().fade(duration: Duration(seconds: 4)),
                                                  SizedBox(width: 8.0),
                                                  Text(
                                                    'H: $tempMax °C',
                                                    style: Theme.of(context).textTheme.bodyMedium!.copyWith(color: Colors.white, fontWeight: FontWeight.bold),
                                                  ).animate().fade(duration: Duration(seconds: 4)),
                                                ],
                                              ),
                                            ),
                                          ),
                                        ],
                                      ),
                                    ),
                                  ),
                                ],
                              ),
                            ),
                          ),
                        ).animate().fade(duration: 1.seconds),
                      ),
                    ),
                    // If iconCode is empty then it is loading state
                    iconCode != ''
                        ? Container(
                            child:
                                Align(
                                      alignment: Alignment.bottomLeft,
                                      child: Container(height: constraints.maxHeight * .5, child: LottieBuilder.asset(getWeatherAsset(iconCode))),
                                    )
                                    .animate(onPlay: (controller) => controller.repeat(reverse: true))
                                    .move(duration: 0.5.seconds, begin: Offset(0, -4), end: Offset(0, -9), curve: Curves.easeInOutSine)
                                    .then()
                                    .move(duration: 0.5.seconds, begin: Offset(0, -2), end: Offset(0, -4), curve: Curves.easeOutSine),
                          ).animate().fade(duration: 1.50.seconds)
                        : Expanded(child: Container()),
                  ],
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }
}
